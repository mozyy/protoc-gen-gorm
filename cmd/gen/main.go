...
func genGeneratedHeader(gen *protogen.Plugin, g *protogen.GeneratedFile, f *fileInfo) {
	...
	if GenerateVersionMarkers {
		g.P("// versions:")
		protocGenGoVersion := "v2.22" // TODO: add version
...
func genMessageDefaultDecls(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo) {
	...
	for _, field := range m.Fields {
		...
		goType, _ := fieldGoType(g, f, field, convGToO)
...
func fieldGoType(g *protogen.GeneratedFile, f *fileInfo, field *protogen.Field,
	...
	case field.Desc.IsMap():
		keyType, _ := fieldGoType(g, f, field.Message.Fields[0], convIdent)
		valType, _ := fieldGoType(g, f, field.Message.Fields[1], convIdent)
...
func genExtensions(g *protogen.GeneratedFile, f *fileInfo) {
	...
	for _, x := range f.allExtensions {
		...
		goType, pointer := fieldGoType(g, f, x.Extension, convGToO)
		if pointer {
			goType = "*" + goType
		}
